...
用户数据全部写在 U 盘根目录的 `zgy-data/`，随 U 盘插拔迁移。
...
- **加载页只等联网校验，几秒就过**：加载页依次显示"正在校验授权 → 正在连接管理后台"；
  若这个盘还没绑定授权，加载页会直接给出**输入授权码**的表单（见第 6 章），填完自动继续；
- **耗时的准备工作全在后台**：运行组件解压（两万多个文件、约半分钟，杀软还要逐个扫）与微信组件首次冷启动都不在启动链上，
  它们和用户登录账号、选模型这一两分钟重叠进行；真正要用到的地方（网关启动、微信/QQ 扫码）各自等同一次解压，
  没准备好时在**面板内**显示进度，不盖全屏加载页，用户可以先去配其它通道；
  解压会跳过 `*.map`、`*.d.ts` 与 linux 预编译件（运行时用不到，少写约 1.2 万个文件）；
  解压缓存按模块包本身（大小 + 修改时间）认键，所以换盘符、换 USB 口、改文件夹名都不会重解压；
- **窗口先行**：加载页在启动一开始就可见，授权校验在后台跑，不会黑等；
...
首次运行的模块解压在后台进行，不挡界面。数据始终写在 exe 同级的 `data/`，跟着 U 盘走。
...
- macOS 必须直接运行 U 盘里的 `.app`，不要拖到 `/Applications` 后运行，否则数据目录不会与 Windows 共用
...
Windows 压缩包也可以直接放在 U 盘后解压到任意子目录；新版客户端会识别实际的可移动/USB 卷，把共享数据统一放在 U 盘根目录的 `zgy-data/`。如果先在本机固定磁盘上测试，则保留在解压目录旁边，不会创建 `C:\zgy-data`。

从旧版本升级时，若发现解压目录旁边已有 `zgy-data/` 或程序目录内已有 `data/`，新版首次启动会把它移动到 U 盘根目录；目标目录已经存在时保留目标目录，不覆盖其中的数据。
...
两种绑定方式，走的是同一条流程（先向后台核对、通过才落本地文件）：

- **界面激活（推荐，单台机器）**：双击程序 → 加载页显示"需要授权，请输入授权码" → 填码点「绑定并继续」。
  macOS 上尤其省事：不用找包内可执行文件、不用过 Gatekeeper、也不用开终端。
- **命令行（批量/脚本）**：

```bash
zgyclaw.exe --bind-usb --license XLX-XXXXXXXX   # 绑定本盘并写入该盘授权码（一码一盘）
...
| --- | --- |
| 通过 | 写本地绑定文件，界面/命令都继续 |
| 后台没有这个授权码 / 已禁用 / 已过期 | **不写文件**，界面显示原因与处理建议，命令退出码 1 |
| 这条授权已绑别的 U 盘（`USB_MISMATCH`） | **不写文件**，提示去后台清空该授权的 U 盘 ID |
| 后台连不上 | 写文件并警告（客户端首次启动时还会再校验一次） |
| 授权码格式非法 | 不联网直接拒绝 |

- 指纹 = `SHA-256(产品盐 + 规范化 USB 硬件序列号)`，Windows 和 macOS 使用同一个硬件序列号命名空间；macOS 按 `system_profiler`、IORegistry 顺序读取，转接器的全零占位序列号会被忽略；
...
- 启动时带授权码 + `USB-硬件序列号` 请求 `/api/client/license/check`，**首次校验会把该盘序列号写进去**，之后换盘返回 `USB_MISMATCH`；要换盘就在后台清空该授权的 U 盘 ID；
...
跨平台授权升级后，旧版本后台记录的 `WINVOL-...` 或 `MACVOL-...` 不是新的 `USB-...` 格式；发布新客户端时，需要先在后台清空该授权的旧 U 盘 ID，再用新客户端在任一平台重新绑定一次，之后 Windows 和 macOS 共用同一个授权文件。

> 本机开发同样走这条路：`npm run dev` 第一次会停在"未写入后台授权码"，
> 在本机执行一次 `npx electron . --bind-usb --license <码>`（绑定文件在 `zgy-data/`，已 gitignore）。
...
所以发版只需两步：把 `package.json` 的 `version` 改掉、打一个同名标签（如 `version` 为 `1.5.0` 就打 `v1.5.0`），
