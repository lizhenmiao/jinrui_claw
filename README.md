...
> 要启动小龙虾客户端和管理后台、做授权配置、打包拷 U 盘这一整套流程，看
> [docs/运行部署手册.md](docs/运行部署手册.md)（含管理后台启动、客户端要对接的外部平台、
> 授权两层机制、更新与模型下发、常见问题与实测记录）。
> 本文其余部分只讲客户端本身的开发与打包。
...
无敏感逻辑，保持源码（渲染进程在启用 asar 完整性校验的包内加载字节码会崩溃，故不编译）。
字节码的保护边界：逻辑与注释受保护，字符串常量仍可提取，授权的强防护依赖后台授权联检
与后续规划的服务端授权租约。
